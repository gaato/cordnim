...
type VoiceBinaryMessage* = object ## Decoded binary Voice Gateway message.
  sequence*: Option[uint16] ## Server sequence number, absent for client messages.
...
proc parseServerBinary*(data: openArray[byte]): VoiceBinaryMessage =
  ...
  if data.len < 3:
    raise newException(ValueError, "server voice binary message must contain sequence and opcode")
...
proc encodeClientBinary*(opcode: VoiceOpcode; payload: openArray[byte]): seq[byte] =
  ...
  if not opcode.isBinary:
    raise newException(ValueError, "voice opcode is not defined as a binary opcode")
