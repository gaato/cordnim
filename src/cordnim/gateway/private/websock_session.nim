## Cordnim-local nim-websock session compatibility layer.
...
import
  ...
  stew/byteutils,
  stew/endians2,
  websock/[frame, types, utf8dfa, http]
...
proc prepareCloseBody(code: StatusCodes, reason: string): seq[byte] =
  # 1005 is an application-side sentinel for a peer close frame with no status
  # payload. RFC 6455 forbids putting that sentinel on the wire; echo an empty
  # close payload instead.
  if code == StatusNoStatus:
    return @[]
...
proc handleClose(
  ...
  of 1:
    ...
    if code == StatusReserved or
      ...
      code == StatusClosedAbnormally or
      code == StatusFailedTls:
      raise newException(WSInvalidCloseCodeError,
        &"Can't use reserved status code: {code}")
    ...
    reason =
      if payload.len > 2:
        string.fromBytes(payload.toOpenArray(2, payload.high))
      else:
        ""

    if validateUTF8(reason) == false:
