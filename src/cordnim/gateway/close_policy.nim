...
type
  GatewayCloseCode* = distinct uint16 ## A Gateway WebSocket close code, including future values.
  ...
  GatewayCloseDecision* = object ## Classified response to a Gateway close code.
    ...
    retryable*: bool ## Whether reconnecting can succeed without operator action.
...
func classifyGatewayClose*(code: GatewayCloseCode): GatewayCloseDecision {.raises: [].} =
  ...
  of 4012:
    GatewayCloseDecision(action: stopReconnecting, reason: "invalid API version")
  of 4013:
